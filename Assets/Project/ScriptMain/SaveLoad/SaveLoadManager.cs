...
public class SaveLoadManager : NetworkBehaviour
{
    ...
    // ── Static-state reset ───────────────────────────────────
    // _pendingRegistrations is static so ISaveables can queue before the
    // SaveLoadManager NetworkObject is spawned.  Without a domain reload,
    // stale destroyed-object references from the previous session accumulate
    // here and are fed into AddToList on the next OnNetworkSpawn, causing
    // MissingReferenceExceptions.  Clear it on every Play-mode entry.
#if UNITY_EDITOR
    ...
    {
        // Try immediately — NGO spawns the player object before firing
        // OnClientConnectedCallback, so PlayerObject should already be ready.
        ...
        // Fallback: wait one frame if PlayerObject wasn't assigned yet.
        yield return null;

        if (!NetworkManager.ConnectedClients.TryGetValue(clientId, out client))
            yield break;
