...
public class SpawnPointManager : MonoBehaviour
{
    ...
    /// <summary>
    /// Returns the spawn Transform at a fixed index — no randomness, does not
    /// advance _nextIndex.  Used by multiplayer connection approval so each
    /// player gets a predictable, sequential slot (Player 0 → SpawnPoint1,
    /// Player 1 → SpawnPoint2, …) without affecting single-player door/dungeon
    /// spawn logic.
    /// </summary>
    public Transform GetMultiplayerSpawnTransform(int playerIndex)
    {
    ...
    /// <summary>
    /// Returns true when an explicit entry point (door or dungeon) is active for
    /// this scene load.  Used by SceneTransitionManager to decide whether all
    /// players should converge on one override point or be spread across
    /// sequential default spawn slots.
    /// </summary>
    public bool HasOverridePoint()
    {
        if (_usingDungeonOverride && dungeonSpawnPoint != null) return true;
        if (!string.IsNullOrEmpty(SceneTransitionManager.LastUsedDoorID))
            return doorSpawnPoints.Exists(d =>
                d.doorID == SceneTransitionManager.LastUsedDoorID && d.spawnPoint != null);
        return false;
    }
