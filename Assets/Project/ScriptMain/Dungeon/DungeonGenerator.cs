...
namespace MyGame.Dungeon
{
    ...
    public class DungeonGenerator
    {
        ...
        // ทิศทางบน/ล่าง/ซ้าย/ขวา สำหรับ cluster BFS
        ...
        /// <summary>
        /// วาง ore เป็น cluster/vein แบบ Stardew Valley
        /// แต่ละ cluster ใช้แร่ชนิดเดียวกัน จำนวนก้อนต่อ cluster = random ระหว่าง
        /// oreClusterMin–oreClusterMax ที่กำหนดใน DungeonConfigSO
        /// </summary>
        private void PlaceOres()
        {
            var avail   = GetFloorTiles(PlayerSpawnPos, LadderPos);
            var orePool = config.GetAvailableOres(floor);
            int total   = config.GetOreCount(floor);

            if (avail.Count == 0 || orePool.Count == 0) return;

            // กรอง tile ที่มี rock อยู่แล้ว
            var rockSet  = new HashSet<Vector2Int>(Rocks);
            avail.RemoveAll(p => rockSet.Contains(p));

            var availSet = new HashSet<Vector2Int>(avail);  // lookup เร็ว
            var usedSet  = new HashSet<Vector2Int>();       // tile ที่จองแล้ว

            Shuffle(avail);   // สุ่มลำดับ seed ของ cluster
            int placed  = 0;
            int seedIdx = 0;

            while (placed < total && seedIdx < avail.Count)
            {
                // หา seed tile ที่ยังไม่ถูกใช้
                while (seedIdx < avail.Count && usedSet.Contains(avail[seedIdx]))
                    seedIdx++;
                if (seedIdx >= avail.Count) break;

                var seed = avail[seedIdx++];

                var ore = config.GetRandomOre(orePool, rng);
                if (ore == null) break;

                // Randomised BFS — ผลลัพธ์เป็น blob cluster รูปทรงอินทรีย์
                int clusterSize = rng.Next(config.oreClusterMin, config.oreClusterMax + 1);
                ...
                {
                    // สุ่มเลือก tile จาก frontier (ไม่ใช่ FIFO — ทำให้ shape เป็นธรรมชาติ)
                    int fi      = rng.Next(0, frontier.Count);
                    var current = frontier[fi];
                    frontier.RemoveAt(fi);
                    cluster.Add(current);

                    foreach (var dir in s_CardinalDirs)
                    {
                        var n = current + dir;
                        if (availSet.Contains(n) && !usedSet.Contains(n))
                        {
                            usedSet.Add(n);
                            frontier.Add(n);
                        }
