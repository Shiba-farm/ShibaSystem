...
public class TrashDropZone : MonoBehaviour, IDropHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    void Start()
    {
        ...
        // cache root canvas สำหรับ animation (fix: tempGO ต้องอยู่ใน Canvas)
    ...
    public void OnDrop(PointerEventData eventData)
    {
        ...
        if (draggedItem != null)
        {
            ...
            // ใช้ inventoryIndex (ตำแหน่งใน NetworkList จริงๆ) ไม่ใช่ slotIndex (visual index)
            // และใช้ RequestDeleteBySlotServerRpc ที่ค้นหาด้วย index โดยตรง แทน itemID
            InventoryNetworkManager.Instance.RequestDeleteBySlotServerRpc(
                draggedItem.sourceSlot.inventoryID,
                draggedItem.sourceSlot.inventoryIndex,
                draggedItem.amount);
    ...
    IEnumerator ShrinkAndSwallowAnimation(Sprite sprite, Vector2 screenPos)
    {
        ...
        // ต้องอยู่ใน Canvas ไม่งั้น RectTransform ไม่ render
        Transform canvasParent = _rootCanvas != null ? _rootCanvas.transform : transform.root;
        RectTransform rect = tempGO.AddComponent<RectTransform>();
        rect.SetParent(canvasParent, worldPositionStays: false);
